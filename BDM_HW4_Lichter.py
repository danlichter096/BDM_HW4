...
def main(sc):
  ...
  a = big_box_df.groupby('year','date').agg(F.collect_list('visits').alias('array_visits'))\
                      .withColumn('median', F.element_at(F.col('array_visits'), F.ceil(F.size(F.col('array_visits'))/2).cast('int'))).drop('array_visits') #F.element_at(F.col('array_visits'), F.ceil((F.size(F.col('array_visits'))/2)).cast('int')))
