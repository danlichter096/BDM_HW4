...
def main(sc):
  ...
  a = big_box_df.groupby('year','date').agg(F.sort_array(F.collect_list('visits')).alias('array_visits'))\
                      .withColumn('middle', F.ceil(F.size(F.col('array_visits'))/2) ).drop('array_visits') #F.ceil((F.size(F.col('array_visits'))/2)).cast('int'))).drop('array_visits')
  ...
  big_box_df.write.option("header", True).csv(f"{a.dtypes}")
