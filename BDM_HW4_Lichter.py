...
def main(sc):
  ...
  a_df=big_box_df.groupBy('year','date').agg(F.stddev_pop('visits').alias('std'), F.percentile_approx(F.col('visits'), 0.5).alias('median')))#, F.sort_array(F.collect_list('visits')).alias('array1'))
